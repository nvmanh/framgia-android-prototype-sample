...
package manhnv.test.data.source.local;
...
import manhnv.test.util.database.DbHelper;
import manhnv.test.util.schedulers.BaseSchedulerProvider;
import rx.Observable;
import rx.functions.Func1;

import static com.google.common.base.Preconditions.checkNotNull;
import static manhnv.test.data.source.local.TasksPersistenceContract.TaskEntry;
...
public class TasksLocalDataSource implements TasksDataSource {
    ...
    private TasksLocalDataSource(@NonNull Context context,
        ...
        mTaskMapperFunction = new Func1<Cursor, Task>() {
            ...
            public Task call(Cursor c) {
                String itemId =
                        c.getString(c.getColumnIndexOrThrow(TaskEntry.COLUMN_NAME_ENTRY_ID));
    ...
    public Observable<Task> getTask(@NonNull String taskId) {
        ...
        String sql =
                String.format("SELECT %s FROM %s WHERE %s LIKE ?", TextUtils.join(",", projection),
